...
const Dropdown = ({ data, placeholder, setItem }) => {
  ...
  return (
    <div className="dropdown">
      <div onClick={() => setOpen(!open)} className="dropdown-selected-item">
        <p className={activeItem === null ? "placeholder" : ""}>
          {activeItem === null ? placeholder : data[activeItem]?.title}
        </p>
      ...
      {data && (
        <ul className={`dropdown-content ${open ? "active" : ""}`}>
          {data?.map((item, index) => (
            <li onClick={() => handleChoseItem(index)} key={item?.key}>
              {item?.title}
            </li>
          ))}
        </ul>
